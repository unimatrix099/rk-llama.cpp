...
> ✅ **Fixed 2026-08-24.** LFM2 rows below were throughput-only and
> quality-invalid for a long time: MoE models produced garbage on the NPU
> (PPL ~17400 vs 15.86 on CPU) because the backend computed only the first
> slice of a batched mul_mat. Now PPL 16.90 at the same speed — see decode
...
| Removing `mallopt(M_MMAP_MAX,0)` | ❌ keep it | crash no longer reproduces without it (26/26 pass incl. the full legacy anchor, vs 0/6 originally) — but peak RSS is bit-identical with and without, so the review's cost argument is empty. The allocation pattern that got hit is gone, not the hazard; librknnrt still unmaps in that range — W4A4 investigation, "Re-tested 2026-08-29" |
